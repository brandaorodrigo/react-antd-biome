...
const codes = [
    ...
    { label: 'Congo (Democratic Republic)', value: 'CD', code: '243' },
    { label: 'Congo (Republic)', value: 'CG', code: '242' },
    ...
    { label: 'Dominican Republic', value: 'DO', code: '1809' },
    ...
    { label: 'Eswatini', value: 'SZ', code: '268' },
...
type InputPhoneProps = {
    allowContries?: CountryCode[];
    defaultCode?: string;
    defaultCountry?: CountryCode;
    national?: boolean;
    onChange?: (value?: string, country?: string, code?: string) => void;
    placeholder?: string;
    readOnly?: boolean;
    value?: string;
};
...
const InputPhone: React.FC<InputPhoneProps> = ({
    ...
    const onChangeInternal = (country: CountryCode, code: string, phone: string) => {
        setLoaded(true);
        ...
        const input = phone.length > 5 ? new AsYouType(country).input(clean) : clean;
        setPhone(input);

        const number = input.replace(/\D/g, '');

        if (country === 'BR') {
            setMinLength(number.length > 2 && number[2] === '9' ? 15 : 14);
        } else {
            setMinLength(10);
        }
        ...
        if (onChange) {
            ...
            if (national) {
                onChange(number, country, code);
                return;
            }
            onChange(`+${code}${number}`, country, code);
        }
    };
    ...
    useEffect(() => {
        ...
        onChangeInternal(
            parse?.country || defaultCountry,
            parse?.countryCallingCode || defaultCode,
            parse ? parse?.formatNational() : value,
        );
    }, [value]);

    useEffect(() => {
        const filtered = allowContries
            ? codes.filter(({ value }) => allowContries.includes(value as CountryCode))
            : codes;
        ...
        setOptions(
            filtered.map((each) => ({
                ...
                label: (
                    <>
                        <Flag countryCode={String(each.value)} svg /> {each.label}
                    </>
                ),
                flag: <Flag countryCode={String(each.value)} svg />,
                code: each.code,
            })),
        );
    }, []);
    ...
    return (
        <Input
            addonBefore={
                !national && (
                    <Select
                        onSelect={(value, option) => onChangeInternal(value, option.code, phone)}
                        optionFilterProp='label'
                        optionLabelProp='flag'
                        options={options}
                        popupClassName='InputPhone'
                        showSearch
                        value={country}
                    />
                )
            ...
            maxLength={15}
            minLength={minLength}
            onChange={(e) => onChangeInternal(country, code, e.target.value)}
